...
class HardBinaryConv(nn.Conv2d):
    def __init__(self, in_chn, out_chn, kernel_size=3, stride=1, padding=0, groups=1, bias=True):
    ...
    def forward(self, x):
        ...
        y = y * scaling_factor.permute(1, 0, 2, 3)
...
class BinaryConv2d(nn.Module):
    ...
    # def forward(self, x):
    #     out = self.relu(self.conv(x)) + x
    #     return out

class BinaryDSConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bias=False):
        super(BinaryDSConv2d, self).__init__()

        self.move0 = LearnableBias(in_channels)
        self.binary_activation0 = BinaryActivation()
        self.binary_dconv = HardBinaryConv(
            in_channels,
            in_channels,
            kernel_size,
            padding=(kernel_size//2),
        ...
        self.binary_pconv = HardBinaryConv(
            in_channels,
            out_channels,
            1,
            bias=bias
    ...
    def forward(self, x):
        out = self.move0(x)
        out = self.binary_activation0(out)
        out = self.binary_dconv(out)
        out = self.relu(out)
        out = self.move1(out)
        out = self.binary_activation1(out)
        out = self.binary_pconv(out)
        if x.shape[1] == out.shape[1]:
            out = out + x
...
class BinaryUpConv2d(nn.Module):
    def __init__(self, in_channels, kernel_size, scale_factor, bias=False):
        super(BinaryUpConv2d, self).__init__()

        self.dsconv = BinaryDSConv2d(in_channels, in_channels * scale_factor**2, kernel_size, bias=bias)
    ...
    def forward(self, x):
        ...
        out = out + F.interpolate(x, scale_factor=self.scale_factor, mode='nearest')
        return out
